...
- **Network response mocking** — `NetworkMockRule`s (URL substring + optional method → status/body/delay), managed from the Network tab's config panel (its tune icon), short-circuit a matching request in both `HttpInterceptor` and `DioNetworkInterceptor` before it reaches the network. Mocked entries are tagged `MOCKED` in the Network tab. Covered by a real integration test on each interceptor — `http.testing.MockClient` for the http client, and a custom `HttpClientAdapter` implementation for Dio (both ship with their respective packages, so no new test dependency) — asserting the underlying client is never actually invoked when a rule matches.
...
- Widened `share_plus` (`<=12.0.0` → `<14.0.0`) and `device_info_plus` (`<=12.0.0` → `<14.0.0`) upper bounds to allow their current latest stable releases (`13.3.0` and `13.2.0` respectively). Neither package's changelog reports a breaking change affecting the APIs this package uses.
